  ...
  return (
    <Document title={report.reference} author={BRANDING.companyName} subject={report.projectName}>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.h2Row} minPresenceAhead={60}>
        ...
        {actions.length === 0 ? (
          ...
          <View>
            <View style={s.thead}>
              <Text style={[s.th, s.cNo, { color: "#ffffff" }]}>REF</Text>
            ...
            {actions.map((observation, index) => (
              <View key={observation.id} style={s.tr} wrap={false}>
                <Text style={[s.td, s.cNo]}>{String(report.observations.indexOf(observation) + 1).padStart(2, "0")}</Text>
          ...
          return (
            <View key={observation.id} style={s.obs}>
              ...
              <View style={[s.obsBody, { borderLeftColor: tone.rule }]}>
                <View style={s.obsHead} wrap={false} minPresenceAhead={40}>
                  <Text style={[s.chipText, { color: tone.ink, marginBottom: 4 }]}>{observation.typeLabel.toUpperCase()}</Text>
                ...
                {observation.actionNeeded.trim() ? (
                  <View style={s.action}>
                    <Text style={s.actionLabel} minPresenceAhead={24}>ACTION REQUIRED</Text>
                ...
                {observation.photos.length > 0 ? (
                  <View style={[s.figs, { marginLeft: 37 }]}>
                    {observation.photos.map((photo, photoIndex) => {
                      ...
                      return (
                        <View key={photo.id} style={style} wrap={false}>
                          ...
                          <Text style={s.capNo}>
                            UPDATE {String(index + 1).padStart(2, "0")} / PHOTOGRAPH {String(figureNumber).padStart(2, "0")}
                          </Text>
